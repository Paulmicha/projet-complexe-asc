...
f_global_write() {
  ...
  echo -n '' > .env
  cat > data/asc/global.vars.sh <<'EOF'
#!/usr/bin/env bash
...
# @see f_global_write() in asc/utilities/global.sh
#
...
EOF
  ...
  for global_name in ${GLOBALS['.sorting']}; do
    ...
    if [[ -z "$global_val" ]]; then
      echo "${readonly}$global_name=''" >> data/asc/global.vars.sh
    else
      if [[ $has_single_quote != 1 ]]; then
        echo "${readonly}$global_name='$global_val'" >> data/asc/global.vars.sh
      else
...
#   # - asc/app/global.compose.vars.sh
#   # - ...
#
f_global_lookup_paths() {
  local f
  ...
  for provision_suffix in $(f_provision_using_lookup_values); do
    hook_dry_run_matches=''
...
f_global_aggregate() {
  ...
  for inc in $global_lookup_paths; do
    ...
    f_autoload_override "$inc" 'continue'
  ...
  if [[ ${GLOBALS['.defer-max']} -gt 0 ]]; then
    ...
    for (( i=1; i<=$max; i++ )); do
      for global_name in ${GLOBALS[".defer-$i"]}; do
        ...
        f_global_assign_value "$global_name"
      done
    done
...
#   # already used internally by ASC :
#   # - 'default'
#   # - 'value'
#   # - 'values'
#   # - 'append'
      ...
      for key in "${!declaration_dict[@]}"; do
        ...
        case "$key" in
          ...
          if-*|ifnot-*)
            ...
            f_str_split1 'depending_var_split_arr' "$key" '-'
            depending_var="${depending_var_split_arr[1]}"

            f_str_sanitize_var_name "$depending_var" 'depending_var'
