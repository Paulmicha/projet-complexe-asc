...
# @see f_db_set()
#
# Every DB variable coming from f_dwt_sites() parsed YAML for given site ID
# serves as preset for f_db_set().
#
...
case "$DWT_MULTISITE" in true)
  ...
  for v in $db_vars_list; do
    ...
    f_str_sanitize_var_name "$dwt_sites_db_var" 'dwt_sites_db_var'
    f_str_lowercase "$dwt_sites_db_var" dwt_sites_db_var
