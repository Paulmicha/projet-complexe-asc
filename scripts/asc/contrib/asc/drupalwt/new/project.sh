...
#   # Or :
#   asc/extensions/drupalwt/new/project.sh thunder/thunder-project 'keep'
#
...
#   # Or :
#   asc/extensions/drupalwt/new/project.sh thunder/thunder-project 'discard'
#
...
if [[ -d "$APP_DOCROOT" ]]; then
  ...
  case "$edd_decision" in
    ...
    'delete')
      echo "  -> Remove all its contents, then re-execute the 'init' hook implementation in the 'git' subject from ASC core."
      rm -rf $APP_DOCROOT/*
      rm -rf $APP_DOCROOT/.* 2>/dev/null
      . asc/git/init.hook.sh
      ;;
